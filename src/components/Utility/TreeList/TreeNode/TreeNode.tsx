...
function TreeNode<T extends Model>(props: TreeNodeProps<T>) {
  ...
  const menuCRUD = React.useMemo(
    () => (
      <Menu>
        {node?.item.level < 4 && validAction('create') && (
          <Menu.Item>
        ...
        {!node?.item?.used && validAction('delete') &&
    ...
    [node, onAdd, onDelete, onEdit, onPreview, validAction]
  );

  const handleShowInput = React.useCallback(
    (node) => () => {
      const newModel = { ...model };
      newModel?.purchaseRequestPlanContents.forEach((item) => {
      ...
      <>
        ...
        >
          ...
          >
            ...
            <div className="tree-node__text-bottom">{node.item.id}</div>
          </div>
        </div>
      </>
    );
