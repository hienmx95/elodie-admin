...
const ContentEditable = forwardRef<HTMLDivElement, ContentEditableProps<ModelFilter>>((props: ContentEditableProps<ModelFilter>, contentEditableRef: MutableRefObject<HTMLDivElement | null>) => {
    ...
    const handleKeyDown = React.useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
        ...
        if (event.key === '@') {
            document.execCommand('insertHTML', false, '<span class="mention-tag">@</span>');
    ...
    const handleKeyUp = React.useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
        if (event.key === 'Enter' && event.shiftKey) {
            ...
            contentEditableRef.current.innerHTML = contentEditableRef.current.innerHTML.trim() + '<br><br>';
            setEndContentEditable();
            return;
        };
    ...
    const handleInput = React.useCallback((event: React.FormEvent<HTMLDivElement>): void => {
        ...
        if (!contentEditableRef.current.innerText || contentEditableRef.current.innerHTML) {
            setShowSuggestList(false);
        }
    ...
    return <>
        <div className="content-editable__container">
            <div className="content-editable__comment"
