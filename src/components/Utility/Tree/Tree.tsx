...
function Tree(props: TreeProps<Model, ModelFilter> & AntdTreeProps) {
  ...
  const searchTreeNode = React.useCallback(
  ...
  const searchTree = React.useCallback(
    (treeNodes: TreeNode<Model>[], listKeys: number[]) => {
      const nodes = [];
  ...
  const handleCheck: AntdTreeProps["onCheck"] = React.useCallback(
    (checkedKeys: { checked: number[]; halfChecked: number[] }) => {
  ...
  React.useEffect(() => {
    if (typeof getTreeData === "function") {
      ...
      getTreeData(modelFilter).subscribe(
        (res: Model[]) => {
          if (res) {
            const [treeData, internalExpandedKeys] = commonWebService.buildTree(
              res,
            );
            if (selectedKey) {
              commonWebService.setDisabledNode(selectedKey, treeData);
            }
            if (onlySelectLeaf) {
              commonWebService.setOnlySelectLeaf(treeData);
            }
