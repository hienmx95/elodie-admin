...
function BrandMaster() {
  ...
  const { validAction } = authenticationService.useAction('product', API_BRAND_PREFIX);

  const master: UseMaster = masterService.useMaster<Brand, BrandFilter>(
    BrandFilter,
    "",
    brandRepository.list,
    brandRepository.count,
    brandRepository.delete, // delete item functions
  ...
  const {
    ...
    handleOpenPreview,
    handleClosePreview,
  } = masterService.usePreview<Brand>(Brand, brandRepository.get);
        ...
        {!brand.used && validAction('delete') && (
          <Menu.Item key="3">
            <Tooltip title={translate("general.actions.delete")}>
              <div
                className="ant-action-menu"
                onClick={() => master.handleServerDelete(brand)}
              >
        ...
        render(...[code]) {
          return (
            <div className="ant-cell-master__container">
              <div className="cell-master__first-row">{code}</div>
        ...
        render(id: number, brand: Brand) {
          return (
            <div className="d-flex justify-content-center button-action-table">
              <Dropdown overlay={menu(id, brand)} trigger={["click"]}>
    ...
    () => (
      <Row className="mt-4 search__container">
        <Col lg={4} className="pr-4">
          <label className="label">{translate("brands.code")}</label>
          <AdvanceStringFilter
        ...
        <Col lg={4} className="pr-4">
          <label className="label">{translate("brands.name")}</label>
          <AdvanceStringFilter
        ...
        <Col lg={4} className="pr-4">
          <label className="label">{translate("brands.description")}</label>
          <AdvanceStringFilter
        ...
        <Col lg={4} className="pr-4">
          <label className="label">{translate("brands.status")}</label>
          <AdvanceIdFilter
