...
import { PurchaseRequestPlanFileMapping } from "models/PurchaseRequestPlanFileMapping/PurchaseRequestPlanFileMapping";

const antIcon = (
  <LoadingOutlined style={{ fontSize: 24, color: "#3F8CFF" }} spin />
);

export function usePurchaseRequestPlanFileMapping(
          ...
          .subscribe(
            (res: FileModel[]) => {
              if (res && res.length > 0) {
                const fileMappings = [];
                res.forEach((current) => {
                  const mappingObj = new PurchaseRequestPlanFileMapping();
...
export interface PurchaseRequestPlanFileMappingModalProps {
  files?: FileModel[];
...
export function PurchaseRequestPlanFileMappingModal(
  props: PurchaseRequestPlanFileMappingModalProps
) {
  const {
    files,
    visibleDialog,
