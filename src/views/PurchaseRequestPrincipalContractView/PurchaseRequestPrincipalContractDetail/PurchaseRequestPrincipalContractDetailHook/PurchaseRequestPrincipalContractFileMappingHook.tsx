...
export function usePurchaseRequestPrincipalContractFileMapping(
          ...
          .subscribe(
            (res: FileModel[]) => {
              if (res && res.length > 0) {
                const fileMappings = [];
                res.forEach((current) => {
                  const mappingObj = new PurchaseRequestFileMapping();
...
export interface PurchaseRequestPrincipalContractFileMappingModalProps {
  files?: FileModel[];
...
export function PurchaseRequestPrincipalContractFileMappingModal(
  props: PurchaseRequestPrincipalContractFileMappingModalProps
) {
  const {
    files,
    visibleDialog,
