...
import { PurchaseRequestPrincipalContractContent } from "../../../../models/PurchaseRequestPrincipalContractContent/PurchaseRequestPrincipalContractContent";
...
interface PurchaseRequestPrincipalContractItemAction {
...
interface PurchaseRequestPrincipalContractItem {
...
function purchaseRequestReducer(
  lastState: PurchaseRequestPrincipalContractItem,
  action: PurchaseRequestPrincipalContractItemAction
): PurchaseRequestPrincipalContractItem {
  switch (action.type) {
...
export function usePurchaseRequestPrincipalContractItem(
  handleMappingItems: (purchaseRequestPrincipalContractContents: PurchaseRequestPrincipalContractContent[]) => void,
  ...
  const [{ itemList, currentItemList, idList, currentIdList, total, checkedAll }, dispatch] = React.useReducer<
    React.Reducer<PurchaseRequestPrincipalContractItem, PurchaseRequestPrincipalContractItemAction>
  ...
  const handleGetItemList = React.useCallback(
    async (filterValue) => {
      ...
      const getNCountItems = forkJoin([
        purchaseRequestPrincipalContractRepository.listItem(filterValue),
        purchaseRequestPrincipalContractRepository.countItem(filterValue),
      ]).pipe(finalize(() => {
        setLoading(false);
      })).subscribe(
        (results: [Item[], number]) => {
  ...
  const handleOpenItem = React.useCallback(
    (principalContractId: number) => (event: any) => {
      if (principalContractId) {
        const itemFilterValue = { ...itemFilter };
        itemFilterValue["principalContractId"]["equal"] = principalContractId;
        dispatchItemFilter({
  ...
  const handleSaveItem = React.useCallback(() => {
    const currentPurchaseRequestContent = purchaseRequestPrincipalContractContents ? [...purchaseRequestPrincipalContractContents] : [];
    const newPurchaseRequestContents: PurchaseRequestContent[] = [...currentItemList]
      .map((currentItem, index) => {
        ...
        if (filteredValue) {
          ...
          purchaseRequestContent.exchangedUnitPrice = purchaseRequestContent?.item?.unitPrice;
          purchaseRequestContent.taxType = purchaseRequestContent?.item?.taxType;
          purchaseRequestContent.taxTypeId = purchaseRequestContent?.item?.taxType?.id;
          purchaseRequestContent.exchangeCurrency = purchaseRequestContent?.item?.currency;
          purchaseRequestContent.exchangeCurrencyId = purchaseRequestContent?.item?.currency?.id;
          purchaseRequestContent.exchangeRate = purchaseRequestContent?.item?.currency?.exchangeRate;
          return purchaseRequestContent;
        }
      });
    handleMappingItems(newPurchaseRequestContents);
    setOpenItemDialog(false);
  }, [currentItemList, purchaseRequestPrincipalContractContents, handleMappingItems]);
  ...
  React.useEffect(() => {
    if (purchaseRequestPrincipalContractContents) {
      const ids = purchaseRequestPrincipalContractContents.length > 0 ?
        purchaseRequestPrincipalContractContents.map(current => current.item?.id) : [];
      const items = purchaseRequestPrincipalContractContents.length > 0
        ? purchaseRequestPrincipalContractContents.map((current) => current.item)
        : [];
      dispatch({
        type: "UPDATE_ID_LIST",
        idList: ids,
  ...
  }, [purchaseRequestPrincipalContractContents]);
...
interface PurchaseRequestPrincipalContractItemModalProps {
  total?: number;
  itemList?: Item[];
  itemFilter?: ItemFilter;
...
export function PurchaseRequestPrincipalContractItemModal(props: PurchaseRequestPrincipalContractItemModalProps) {
  const {
    total,
    itemList,
    itemFilter,
          ...
          <div className="item-dialog__container">
            <div className="item-dialog__filter">
              <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                ...
                <Col className="gutter-row" span={6}>
                  <AdvanceStringFilter
                    value={itemFilter[nameof(itemList[0].otherName)]["startWith"]}
                ...
                <Col className="gutter-row" span={6}>
                  <AdvanceIdFilter
                    ...
                    isEnumList={true}
                    getList={purchaseRequestPrincipalContractRepository.singleListItemSelectOption}
                  />
                </Col>
              </Row>
            </div>
