...
} from "../PurchaseRequestDetail/PurchaseRequestDetailHook/PurchaseRequestFileMappingHook";
import { PurchaseRequestWorkflowHistoryModal, usePurchaseRequestWorkflowHistory } from "../PurchaseRequestDetail/PurchaseRequestDetailHook/PurchaseRequestWorkflowHistoryHook";
import "./PurchaseRequestPreview.scss";
...
function PurchaseRequestPreview() {
  ...
  React.useEffect(() => {
    if (model.purchaseRequestStatusId === 2) {
      ...
      purchaseRequestRepository.listPurchasePlan(purchasePlanFilter)
        .subscribe(res => { setListPurchasePlan(res); });
    }
  }, [setListPurchasePlan, model.id, model.purchaseRequestStatusId]);

  const handlePrint = React.useCallback(() => {
    purchaseRequestRepository.print(model).subscribe(
      () => {
        notifyUpdateItemSuccess();
      },
      (err) => {
