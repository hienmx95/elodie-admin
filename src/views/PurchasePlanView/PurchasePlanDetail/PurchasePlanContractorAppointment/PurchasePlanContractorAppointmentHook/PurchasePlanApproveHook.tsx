...
export function useComparision(
  ...
  const handleGoPreStep = React.useCallback(
    (model) => {
      history.replace(
        `${PURCHASE_PLAN_ROUTE}${baseRoute}-detail?id=${model?.id}&purchasePlanTypeId=${model?.purchasePlanTypeId}`
      );
    },
    [baseRoute, history]
  );
    ...
    Modal.confirm({
      ...
      okText: (
        <span>
          ...
          {actionString ? actionString : "Tính điểm"}
        </span>
      ),
      onOk() {
  ...
  }, [actionString, handleOpenDetailModal, model]);
  ...
  const handleOpenConfirmRedoSupplier = React.useCallback(() => {
    Modal.confirm({
      className: "modal-purchase-plan-type__confirm",
      title: "",
      icon: "",
        ...
        <>
          <div className="confirm d-flex align-items-center justify-content-center mt-3">
            <img
              ...
              width="167"
            />
            <div className="confirm-title mt-3">Chỉ định lại Nhà cung cấp!</div>
            <div className="confirm-content mt-3">
              Toàn bộ thông tin báo giá sẽ bị xoá khi thay đổi Nhà cung cấp chỉ
              định. Hãy chắc chắn bạn muốn thay đổi Nhà cung cấp!
            </div>
          </div>
        </>
      ),
      okText: (
        <span>
          <i className="tio-checkmark_circle_outlined" /> Thay đổi
        </span>
      ),
      ...
      onOk() {
        purchasePlanRepository.reset(model).subscribe(
          () => {
            notifyUpdateItemSuccess();
            handleGoPreStep(model);
          },
          () => {
            notifyUpdateItemError(); // global message service go here
          }
  ...
  }, [handleGoPreStep, model, notifyUpdateItemError, notifyUpdateItemSuccess]);

  return {
    handleOpenConfirmModal,
    handleOpenConfirmReplay,
    handleOpenConfirmCancel,
    handleOpenConfirmRedoSupplier,
  };
}
