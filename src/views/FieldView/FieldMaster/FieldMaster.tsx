...
import FieldDetailModal from "../FieldDetail/FieldDetailModal";
...
function FieldMaster() {
  ...
  const menuAction = React.useCallback((id: number, field: Field) => (
    <MenuAntd>
      <MenuAntd.Item key="1">
      ...
      <MenuAntd.Item key="3">
        <Tooltip title={translate("general.actions.delete")}>
          <button className="btn btn-sm component__btn-delete"
            onClick={() => handleServerDelete(field)}>
  ...
  const columns: ColumnProps<Field>[] = useMemo(
    () => [
      ...
      {
        title: (<div className='text-center gradient-text'>{translate('fields.name')}</div>),
      ...
      {
        title: (<div className='text-center gradient-text'>{translate('fields.isDeleted')}</div>),
      ...
      {
        title: (<div className='text-center gradient-text'>{translate('fields.fieldType')}</div>),
      ...
      {
        title: (<div className='text-center gradient-text'>{translate('fields.menu')}</div>),
        ...
        render(id: number, field: Field) {
          return (
            <div className='d-flex justify-content-center button-action-table'>
              <Dropdown overlay={menuAction(id, field)} trigger={["click"]} placement="bottomCenter" arrow>
