...
export class ProductGroupingRepository extends Repository {
    ...
    public countProduct = (productFilter?: ProductFilter): Observable<number> => {
        return this.httpObservable.post<number>(kebabCase(nameof(this.countProduct)), productFilter)
            .pipe(map((response: AxiosResponse<number>) => response.data));
    };

    public listProduct = (productFilter?: ProductFilter): Observable<Product[]> => {
        return this.httpObservable.post<Product[]>(kebabCase(nameof(this.listProduct)), productFilter)
            .pipe(map((response: AxiosResponse<Product[]>) => response.data));
    };
    public deleteProduct = (product: Product): Observable<Product> => {
        return this.httpObservable.post<Product>(kebabCase(nameof(this.deleteProduct)), product)
            .pipe(map((response: AxiosResponse<Product>) => response.data));
    };
    public bulkDeleteProduct = (idList: number[] | string[]): Observable<void> => {
        return this.httpObservable.post(kebabCase(nameof(this.bulkDeleteProduct)), idList)
            .pipe(map((response: AxiosResponse<void>) => response.data));
    };
