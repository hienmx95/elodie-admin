...
import { AppAction, AppActionEnum, AppState } from "app/app-store";
import { AppMessageContext, AppStoreContext } from "app/app-context";
...
export const importExportDataService = {
  ...
  useImport<T extends Model>(
    ...
    const handleImportContentList = useCallback(
      (
        modelId: number,
        onImport: (file: File, name: number) => Observable<T[]>
      ) => {
        return (event: ChangeEvent<HTMLInputElement>) => {
          const file: File = event.target.files[0];
          dispatch({ type: AppActionEnum.INIT_IMPORT }); // dispatch appAction
    ...
    const handleImportTreeContentList = useCallback(
      (onImport: (file: File) => Observable<any>) => {
        return (event: ChangeEvent<HTMLInputElement>) => {
          const file: File = event.target.files[0];
          dispatch({ type: AppActionEnum.INIT_IMPORT }); // dispatch appAction
          if (typeof onImport === "function") {
              ...
              onImport(file).subscribe(
                (list: any) => {
                  ...
                  if (typeof onImportSuccess === "function") {
                    ...
                    newModel[fieldName] = list[fieldName];
                    onImportSuccess(newModel);
                  }
                },
                handleImportError, // onError
                handleEndImport //finally
